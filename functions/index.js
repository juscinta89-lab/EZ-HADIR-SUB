...
/* ── ujian: hantar notifikasi serta-merta kepada pemanggil ────
   Fungsi ini TIDAK melontar ralat. Ia sentiasa memulangkan
   laporan supaya app boleh memaparkan sebab sebenar kegagalan. */
exports.ujiNotifikasi = onCall(async (req) => {
  const laporan = { ok: false, langkah: 'mula' };
  try {
    const emel = (req.auth?.token?.email || '').toLowerCase();
    if (!emel) return { ...laporan, ralat: 'Belum log masuk.' };
    laporan.emel = emel;

    const sid = String(req.data?.sid || '').trim();
    if (!sid) return { ...laporan, ralat: 'sid tidak dihantar oleh app.' };
    laporan.sid = sid;

    laporan.langkah = 'baca token';
    const snap = await db.collection('sekolah').doc(sid)
      .collection('token').where('emel', '==', emel).get();
    laporan.jumlahToken = snap.size;
    if (snap.empty) {
      return { ...laporan, ralat: 'Tiada token berdaftar untuk ' + emel + ' di ' + sid +
        '. Matikan dan hidupkan semula Peringatan harian.' };
    }
    ...
    laporan.kodRalat = res.responses
      .filter(r => !r.success)
      .map(r => r.error?.code || 'tidak diketahui')
      .slice(0, 3);

    // buang token yang sudah mati supaya tidak berulang
    const buang = [];
    res.responses.forEach((r, i) => {
      const kod = r.error?.code || '';
      if (!r.success && (kod.includes('registration-token-not-registered') ||
                         kod.includes('invalid-argument'))) {
        buang.push(snap.docs[i].ref.delete());
      }
    });
    if (buang.length) await Promise.allSettled(buang);

    laporan.ok = res.successCount > 0;
    laporan.langkah = 'siap';
    console.log('Ujian:', JSON.stringify(laporan));
    return laporan;

  } catch (e) {
    console.error('Ujian gagal:', e);
    return {
      ...laporan,
      ralat: (e?.code ? e.code + ' — ' : '') + (e?.message || String(e))
    };
  }
});
